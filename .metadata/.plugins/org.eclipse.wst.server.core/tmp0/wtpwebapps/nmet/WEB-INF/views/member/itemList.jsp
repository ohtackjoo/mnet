...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(document).on('click','.itemtr', function(){
			itemcd=$(this).find("td:eq(0)").html();
			itemname=$(this).find("td:eq(1)").html();
			madenmcd=$(this).find("td:eq(2)").html();
			itemunitcd=$(this).find("td:eq(4)").html();
			$("#itemFrom").show();
			$("#itemcode").val(itemcd);
			$("#itemname").val(itemname);
			$("#madename option").each(function(idx,item){
				if($(this).val()==madenmcd){
					$(this).attr("selected","selected");
				}
			});
			$("#itemunitname option").each(function(){
				if($(this).val()==itemunitcd){
					$(this).attr("selected","selected");
				}
			$("#itemname").attr("readonly", true);
			$("#madename").attr("disabled",true);
			$("#itemunitname").attr("disabled",true);
			});		
		});
		$("#show").click(function(){
			$("#inorup").val("up");
			$("#itemcode").parent().show();
			$("#itemname").val(itemname);			
			$("#itemname").attr("readonly", false);
			$("#madename").attr("disabled",false);
			$("#itemunitname").attr("disabled",false);
			return false;
		});
...
<form action="itemList.do" method="post" id="itemFrom">
	<table id="itemlict">
		...
		<tr>
			<td>
				제조사:<select id="madename" name="madename" disabled="disabled">
				<c:forEach var="made" items="${clist2 }">
					<option value="${made.CDNO}">${made.CDNAME }</option>
				</c:forEach>
				</select>
				
				단위명:<select id="itemunitname" name="itemunitname" disabled="disabled">
				<c:forEach var="item" items="${clist1 }">
					<option value="${item.CDNO}">${item.CDNAME }</option>
				</c:forEach>
				</select>
			</td>
		</tr>
