...
<script type="text/javascript">
	$(document).ready(function(){
		...
		var userparam1=$("#userparam1").val();
		console.log(userparam);
		if(userparam!=""){
			send1(0);
			send(0);
		}
		console.log(userparam1);
		$("#itemFrom").hide();
		$(".hide").hide();
	...
	function send1(i) {
		var code="";
		var userparam=$("#userparam").val();
		console.log("=>"+$("#codeSel option:selected").val());
		console.log("=>"+code);
		console.log("userparam === " + userparam);
		if(i>0){
			code=$("#codeSel option:selected").val();
		}else{
			code=$("#userparam").val();			
		}
		console.log("==>"+code);
		$.ajax({
			...
			success:function(data){
	            ...
	            for(var i=0;i<data.alist.length;i++){
	            	...
	            	if(userparam1==cdno){
	            		$("#ajaxsel option").attr("selected","selected");
	            		send();
	            	}
	            }
	            console.log("success");
	...
	function send(i) {
			...
			var userparam1=$("#userparam1").val();
			if(i>0){
				code=$("#ajaxsel option:selected").val();
			}else{
				code=$("#userparam1").val();
			}				
			console.log(code);
...
<h1>전체리스트</h1>
		카테고리:
			<select id="codeSel" onchange="send1(1)">
			...
			<input type="button" value="조회" id="itemselect" onclick="send(1)">
<table id="itemsel">
	<thead>
		<tr>
			<th>상품코드
...
<form action="insitemlist.do" method="post" id="itemFrom">
	<table id="itemlict">
		...
		<tr>
			...
			<td><input type="hidden" name="userparam" id="userparam"readonly="readonly" value="${userparam }">
			<td><input type="hidden" name="userparam1" id="userparam1"readonly="readonly" value="${userparam1 }">
		<tr>
			<td>
